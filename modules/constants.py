...
from ids.items import itm_spice, itm_siege_supply
...
# consumer demand for a good in town, measured in abstract units. The more essential the item (ie, like grain) the higher the price
...
# reserve around 10 slots after this MOTO actually more than 30 for BW
slot_item_multiplayer_faction_price_multipliers_begin = 70
...
slot_item_base_price = 53  # taken from items
...
# the last time that the faction has had default or feast AI -- this determines lords' dissatisfaction with the campaign. Set during faction_ai script
...
# collected automatically by NPC lords #TEMPERED chief USED ON LOOT WAGON TO STORE GOLD AFTER TOWN TRADE
...
# a harmless number, until it can be deprecated, motomataru, it is native, are you sure to change?
...
# pasture area for grazing of cattles and sheeps, if this value is high then number of cattles and sheeps increase faster
...
# right now used only within a single script, merchant_road_info_to_s42, to denote closed roads. Now also used in comparative scripts
...
# This goes into constants.py, player slots. multiplayer chief
...
# Usually inactive (Calradia is a traditional place). However, can be made potentially active if active_npcs are expanded to include ladies
slto_kingdom_lady = 6
...
# in traveller attacked, origin and destination are center and troop object, and the attacker's faction is the object
...
# coldblooded, moralizing, often cruel - eg William the Conqueror, Timur, Octavian, Aurangzeb (although he is arguably upstanding instead, particularly after his accession)
lrep_selfrighteous = 3
...
# This prevents recruiters from going to villages targeted by other recruiters.
dplmc_slot_village_reserved_by_recruiter = 235
# Alkhadias Master, you forgot this one from the PM you sent me :D
dplmc_slot_party_recruiter_needed_recruits_faction = 236
...
# e.g. father for x = (DPLMC_VIRTUAL_RELATIVE_MULTIPLIER * x) + DPLMC_VIRTUAL_RELATIVE_FATHER_OFFSET
DPLMC_VIRTUAL_RELATIVE_FATHER_OFFSET = -1
DPLMC_VIRTUAL_RELATIVE_MOTHER_OFFSET = -2
DPLMC_VIRTUAL_RELATIVE_SPOUSE_OFFSET = -3
# diplomacy chief end

# TEMPERED chief PARTY SLOTS
# number range from zero, used to check if spy is in a town and how long he has been there in hours.
slot_spy_in_town = 300
...
# for messenger scripts, stores 2nd party in message, such as a group to attack or town to meet at.
slot_message_target_2 = 310
...
# indicates a lord that has challenged player to duel, stores time + 24 hours, indicating time player has to arrive at duel.
slot_troop_duel_challenger = 167
# indicates a lord that was challenged by player to duel, stores time + 24 hours, indicating time player has to arrive at duel.
slot_troop_duel_challenged = 168
...
coop_temp_casualties_enemy_begin = coop_temp_party_enemy_begin + \
    40  # 4x this number = 160 total temp parties reserved in MP
...
# size above which AI will      make a (more maneuverable) square (typical Roman lines 40 w x 7.5      d; here 16 w x 3 d)
AI_Max_Size_Oblong_Formations = 48
