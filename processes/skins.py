...
class SkinProcessor(ModuleProcessor):
  clean_ids_file = True
  export_name = "skins.txt"

  def after_open_export_file(self):
    self.export_file.write("skins_file version 1\n")
    self.export_file.write("%d\n" % len(skins))

  def write_export_file(self, skin):
    save_skin(self.export_file, skin)
