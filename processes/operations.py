...
def process_param(param, local_vars_list, local_var_uses):
  result = 0
  if (isinstance(param, str)):
    if (param[0] == '$'):
      check_varible_not_defined(param[1:], local_vars_list)
      result = get_variable(param, variables, variable_uses)
      result |= opmask_variable
    elif (param[0] == ':'):
      check_varible_not_defined(param[1:], variables)
...
def save_statement(ofile, opcode, no_variables, statement, local_vars_list, local_var_uses):
  if no_variables == 0:
  ...
  for i in range(lenstatement):
    operand = process_param(statement[i + 1], local_vars_list, local_var_uses)
...
def save_statement_block(ofile, statement_name, can_fail_statement, statement_block):
  local_vars = []
  local_var_uses = []
...
#COOP BEGIN WSE chief#############
    if (opcode in [try_begin,
      ...
      print("WARNING: Script can fail at operation #" + str(i) +
            ". Use cf_ at the beginning of its name: " + statement_name)
    save_statement(ofile, opcode, no_variables, statement, local_vars, local_var_uses)
...
def compile_global_vars(statement_block, variables, variable_uses):
  for statement in statement_block:
    compile_global_vars_in_statement(statement, variables, variable_uses)
