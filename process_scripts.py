...
from process_common import convert_to_identifier, lf_open
...
def save_scripts(variable_list, variable_uses, scripts, tag_uses, quick_strings):
    ...
    for i_script in range(len(scripts)):
        ...
        if (type(func[1]) == list_type):
            ...
            save_statement_block(file, convert_to_identifier(
                func[0]), 0, func[1], variable_list, variable_uses, tag_uses, quick_strings)
        else:
            file.write("%s %f\n" % (convert_to_identifier(func[0]), func[1]))
            save_statement_block(file, convert_to_identifier(
                func[0]), 0, func[2], variable_list, variable_uses, tag_uses, quick_strings)
        file.write("\n")
    file.close()


def save_python_header():
    file = lf_open("./ID_scripts.py", "w")
    for i_script in range(len(scripts)):
